...
export function exportGame(state: GameState) { const url = URL.createObjectURL(new Blob([JSON.stringify(state, null, 2)], { type: 'application/json' })); const a = document.createElement('a'); a.href = url; a.download = 'room-economy-save.json'; a.click(); URL.revokeObjectURL(url); }
