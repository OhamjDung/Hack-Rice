...
try{
 ...
 await wait('document.querySelector(".room-scene").dataset.sprites==="ready"');
 await new Promise(r=>setTimeout(r,500));
 let shot=await send('Page.captureScreenshot',{format:'png'});await writeFile(path.join(output,'cashbound-sprites-desktop.png'),Buffer.from(shot.data,'base64'));
 const canvasData=await evaluate('document.querySelector("canvas").toDataURL("image/png")');await writeFile(path.join(output,'cashbound-sprites-room.png'),Buffer.from(canvasData.split(',')[1],'base64'));
 await send('Emulation.setEmulatedMedia',{features:[{name:'prefers-reduced-motion',value:'reduce'}]});await send('Page.reload');await ready();await wait('document.querySelector(".room-scene").dataset.sprites==="ready"');
 for(const [name,sx,sy] of [['fridge',480,200],['kitchen',380,280],['desk',250,340],['bed',755,360],['table',523,399],['sofa',592,435],['keys',399,480]]){
  const cx=640+(sx-529)*24/31*1.3,cy=480+((sy-207)*24/31-90)*1.3;
  ...
  await send('Input.dispatchMouseEvent',{type:'mousePressed',...point,button:'left',clickCount:1});await send('Input.dispatchMouseEvent',{type:'mouseReleased',...point,button:'left',clickCount:1});await wait('!!document.querySelector("dialog[open]")');console.log(name+': '+await evaluate('document.querySelector("dialog h2").textContent'));await close();
 }
 await aria('Zoom in');await aria('Zoom in');
 for(const key of ['ArrowLeft','ArrowRight','ArrowUp','ArrowDown','Home']){
  await evaluate('document.querySelector("canvas").dispatchEvent(new KeyboardEvent("keydown",{key:'+JSON.stringify(key)+',bubbles:true}))');await new Promise(r=>setTimeout(r,100));
  assert.ok(await evaluate('(()=>{const c=document.querySelector("canvas"),p=c.getContext("2d").getImageData(0,0,c.width,c.height).data;for(let x=0;x<c.width;x++)if(p[x*4+3]||p[((c.height-1)*c.width+x)*4+3])return false;for(let y=0;y<c.height;y++)if(p[y*c.width*4+3]||p[(y*c.width+c.width-1)*4+3])return false;return true;})()'),'Unclipped room');
 }
 await send('Emulation.setDeviceMetricsOverride',{width:390,height:844,deviceScaleFactor:1,mobile:true});await new Promise(r=>setTimeout(r,700));
 shot=await send('Page.captureScreenshot',{format:'png'});await writeFile(path.join(output,'cashbound-sprites-mobile.png'),Buffer.from(shot.data,'base64'));
 assert.equal(await evaluate('document.documentElement.scrollWidth<=innerWidth'),true);
 assert.deepEqual(errors,[]);console.log('PASS: source sprites on desktop/mobile, all seven furniture interactions, pan/zoom, no runtime errors.');
