...
export async function GET() {
  ...
  try {
    const result = await db().query<{ username: string; visit_x: number | null; visit_y: number | null }>(
      "SELECT u.username, p.visit_x, p.visit_y FROM presence p JOIN users u ON u.id = p.user_id WHERE p.visiting = $1 AND p.last_seen > now() - interval '20 seconds'",
      [userId],
    );
    return Response.json({ success: true, data: result.rows.map(r => ({ username: r.username, x: r.visit_x, y: r.visit_y })) });
  } catch (e) {
