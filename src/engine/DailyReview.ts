...
export const analysisSchema=z.object({isWarning:z.boolean(),riskRating:z.enum(['LOW','MEDIUM','HIGH','CRITICAL']),commentary:z.string().min(1).max(160),reason:z.string().max(1500),recoverySteps:z.array(z.string().max(400)).min(1).max(4),behavior:z.enum(['calm','worried','restless','tired']),ending:z.enum(['none','food_shortage','power_cut','eviction','exhaustion']).default('none'),failureConfidence:z.number().min(0).max(1).default(0)});
...
export function localDailyReview(state:GameState,kind:'preview'|'close'):DailyReview{
 ...
 const analysis:DailyAnalysis={isWarning:warning,riskRating:state.metrics.cashBalance<forecast.essentialsRemaining?'HIGH':warning?'MEDIUM':'LOW',commentary:warning?'OMG, yesterday I spoiled my shopping bags. Now rent wants a word.':'Yesterday I kept my wallet out of trouble. Character development!',reason:warning?forecast.riskReasons.join(' '):'Your current spending pace leaves room for essentials before the next paycheck.',recoverySteps:warning?['Pause optional purchases and protect money for rent, food, and utilities.','Check the budget notebook before your next treat.']:['Keep essential bill money reserved.'],behavior:warning?'worried':'calm',ending:'none',failureConfidence:0};
...
export function applyDailyReview(state:GameState,review:DailyReview):GameState{
 ...
 if(!s.isGameOver){s.player.targetPosition=a.behavior==='tired'?{x:3,y:7}:a.isWarning?{x:8,y:2}:{x:5,y:4};s.player.state=a.behavior==='tired'?'sleeping':a.isWarning?'worried':'idle';}
 return s;
}
