...
export default function Game({ initialGame }: { initialGame?: GameState }) {
    ...
    return <div className={`game-world ${!game.life.powerOn?'blackout':''} ${game.life.stress>=35?'stressed':''}`} data-testid="game-world">
      ...
      <div className="world-stage"><RoomCanvas key={run} game={game} onLayoutChange={roomLayout=>setGame(s=>({...s,roomLayout}))} onInspect={setDialog} onScene={line=>{setSceneLine(line);setGame(s=>appendDialogue(s,line));}} onEndingComplete={()=>setGame(s=>finishEnding(s))} onSwipeCharacter={()=>{setVisitError('');setVisitDialogOpen(true);}} paused={!!dialog}/></div>
